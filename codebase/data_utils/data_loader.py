...
def get_pairs_from_paths_new(images_path, segs_path, indices=None,ignore_non_matching=False):
    """ Find all the images from the images_path directory and
        the segmentation images from the segs_path directory
        while checking integrity of data """
    ...
    # image_files = [os.path.splitext(dir_entry) + (os.path.join(images_path, dir_entry),)
    #                for dir_entry in os.listdir(images_path)
...
class ImageSegmentationGen:
    ...
    def __init__(self, images_path, segs_path, batch_size,
                 ...
                 output_width, do_augment=False, sampler=None):

        """"added sampler for path partitioning into validation/training"""
    ...
    def __next__(self):
        with self.lock:
            ...
            if self.sampler is not None:
                sampler = list(self.sampler.file_id)
                img_seg_pairs = [img_seg_pair for img_seg_pair in img_seg_pairs if
                                 os.path.splitext(os.path.basename(img_seg_pair[0]))[0] in sampler]
